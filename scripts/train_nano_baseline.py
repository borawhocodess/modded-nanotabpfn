...
class TabPFNBaseline:
    def __init__(
        ...
        *,
        ...
        self.feature_preprocessor = None
        kwargs = dict(inference_config=inference_config, n_estimators=n_estimators, random_state=random_state)
        if version is not None:
            self.clf = TabPFNClassifier.create_default_for_version(version, **kwargs)
        else:
            self.clf = TabPFNClassifier(**kwargs)  # default: TabPFN 2.5 weights
...
def make_no_preprocessing_inference_config() -> InferenceConfig:
...
for epoch in range(1, c.epochs + 1):
    ...
    aucs = []
    run_baselines = epoch == 1
    baseline_models = (
        {
            "random_forest": RandomForestBaseline(random_state=c.seed),
            "tabpfn_v2": TabPFNBaseline(version=ModelVersion.V2, random_state=c.seed),
            "tabpfn_v2_no_preprocessing": TabPFNBaseline(
                version=ModelVersion.V2,
                inference_config=make_no_preprocessing_inference_config(),
                n_estimators=1,
                random_state=c.seed,
            ),
            "tabpfn_v2_5": TabPFNBaseline(random_state=c.seed),
            "tabpfn_v2_5_no_preprocessing": TabPFNBaseline(
                inference_config=make_no_preprocessing_inference_config(),
                n_estimators=1,
                random_state=c.seed,
            ),
    ...
    baseline_aucs = {name: [] for name in baseline_models}
    ...
    for task_id in TABARENA_CLASSIFICATION_TASKS:
        ...
        targets = []
        probabilities = []
        baseline_targets = {name: [] for name in baseline_models}
        baseline_probabilities = {name: [] for name in baseline_models}
        ...
        for _, (train_indices, test_indices) in enumerate(cv.split(X, y)):
            ...
            for baseline_name, baseline in baseline_models.items():
                ...
                baseline_y_proba = baseline.predict_proba(X_test)
                if baseline_y_proba.shape[1] == 2:
                    baseline_y_proba = baseline_y_proba[:, 1]
                baseline_targets[baseline_name].append(y_test)
                baseline_probabilities[baseline_name].append(baseline_y_proba)
        ...
        for baseline_name in baseline_models:
            baseline_y_true = np.concatenate(baseline_targets[baseline_name], axis=0)
            baseline_y_proba = (
                np.concatenate(baseline_probabilities[baseline_name], axis=0)
                if len(baseline_probabilities[baseline_name]) > 0
                else None
            )
            baseline_auc = (
                roc_auc_score(baseline_y_true, baseline_y_proba, multi_class="ovr")
                if getattr(baseline_y_proba, "ndim", 1) > 1
                else roc_auc_score(baseline_y_true, baseline_y_proba)
            )
            baseline_aucs[baseline_name].append(baseline_auc)

    avg_auc = (sum(aucs) / len(aucs)) if len(aucs) > 0 else float("nan")
    for baseline_name, scores in baseline_aucs.items():
        avg_baseline_auc = (sum(scores) / len(scores)) if len(scores) > 0 else float("nan")
        print0(f"avg_{baseline_name}_roc_auc:{avg_baseline_auc}", console=True)
