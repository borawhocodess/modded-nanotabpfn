...
\begin{frame}{Record 2 --- Muon optimizer \hfill \mut{54.41 min}}
  \begin{itemize}
    \item \hl{Muon} = MomentUm Orthogonalized by Newton--Schulz.\\
      Applies to the 2D hidden weight matrices; embeddings, head, and scalars stay on AdamW.
    \medskip
    \item Replaces the momentum update with its nearest semi-orthogonal matrix ($UV^\top$) --- amplifying the rare-but-important directions a few dominant ones would otherwise drown out.
    \medskip
    \item Slower per step, but far fewer steps $\to$ net wallclock win.
    \item A general optimizer, nothing tabular-specific --- it carries straight over. \cit{Jordan '24}
  \end{itemize}
\end{frame}
...
\begin{frame}{The modded architecture}
  \centering
  \includegraphics[width=0.95\textwidth]{img/base-correct.png}\\[3mm]
