...
\begin{column}{0.46\paperwidth}
  ...
  \begin{block}{Improvement techniques}
    ...
    \begin{itemize}
      ...
      \item Increase embedding size from 192 to 256, reduce attention heads from 6 to 4
      \item Exponential decay of the residual stream across layers
      \item Lower precision RMSNorm \cite{zhang2019rmsnorm}
      \item Prepend 24 learnable Thinking Rows \cite{grinsztajn2026tabpfn25advancingstateart}
      \item Latest Weight Averaging of 10 checkpoints \cite{kaddour2022lawa, sanyal2024early}
      \item Set Schedule-Free AdamW weight decay to 0.01 \cite{defazio2024schedulefree}
      \item Repeated feature grouping (group size 5) \cite{qu2026tabiclv2}
      \item Reduce transformer layers from 6 to 5, increase batch size from 1 to 2
      \item Add decoupled weight decay 0.1 to Muon, increase momentum from 0.95 to 0.96
      \item Increase gradient clip from 1.0 to 2.0
      \item Feed mean of test feature embeddings into output decoder
      \item Autoresearch-driven HPO and architecture search \cite{karpathy2026autoresearch}
    \end{itemize}
...
\begin{column}{0.44\paperwidth}
  ...
  \begin{block}{Record history}
    ...
    \begin{figure}
      \centering
      \includegraphics[width=0.95\linewidth]{img/260529-185957-records-plot-x-datasets-y-val.png}
