...
class Config:
    ...
    h: int = 768
    l: int = 6
    encoder_impl: str = "sdpa_qkv"
    autocast_train: bool = True
    autocast_eval: bool = True
    matmul_precision_high: bool = True
    o: int | None = None
    eval_folds: int = 5
...
PRESET_OVERRIDES = {
    "muon": {
        ...
        "autocast_eval": False,
        "matmul_precision_high": False,
    },
    "carter": {
        "lr": 1e-3,
        "a": 4,
        "e": 256,
        "encoder_impl": "sdpa_qkv",
        "autocast_train": True,
        "autocast_eval": True,
        "matmul_precision_high": True,
    },
}
...
parser.add_argument("--preset", choices=("muon", "carter"), default="carter")
...
parser.add_argument("--max-train-mins", type=float, default=None)
parser.add_argument("--encoder-impl", choices=("mha", "sdpa_qkv"), default=None)
parser.add_argument("--autocast-train", choices=("on", "off"), default=None)
parser.add_argument("--autocast-eval", choices=("on", "off"), default=None)
parser.add_argument("--matmul-precision-high", choices=("on", "off"), default=None)
args = parser.parse_args()

c = Config()
for key, value in PRESET_OVERRIDES[args.preset].items():
    setattr(c, key, value)
...
if args.max_train_mins is not None:
    c.max_train_mins = args.max_train_mins
if args.encoder_impl is not None:
    c.encoder_impl = args.encoder_impl
if args.autocast_train is not None:
    c.autocast_train = parse_on_off(args.autocast_train)
if args.autocast_eval is not None:
    c.autocast_eval = parse_on_off(args.autocast_eval)
if args.matmul_precision_high is not None:
    c.matmul_precision_high = parse_on_off(args.matmul_precision_high)

random.seed(c.seed)
np.random.seed(c.seed)
torch.manual_seed(c.seed)
if c.matmul_precision_high:
...
class NanoTabPFNModel(nn.Module):
    def __init__(self, l, a, e, h, o, encoder_impl):
        """
        l : num layers
        ...
        self.transformer_encoder = TransformerEncoderStack(l, a, e, h, encoder_impl=encoder_impl)
...
class TransformerEncoderStack(nn.Module):
    def __init__(self, l, a, e, h, encoder_impl):
        super().__init__()
        self.transformer_blocks = nn.ModuleList()
        for _ in range(l):
            self.transformer_blocks.append(TransformerEncoderLayer(a, e, h, encoder_impl=encoder_impl))
...
class TransformerEncoderLayer(nn.Module):
    def __init__(self, a, e, h, encoder_impl="sdpa_qkv", eps=1e-5):
        super().__init__()
        if encoder_impl not in {"mha", "sdpa_qkv"}:
            raise ValueError(f"unknown encoder_impl: {encoder_impl}")
        self.encoder_impl = encoder_impl

        if self.encoder_impl == "mha":
            self.a_datapoints = nn.MultiheadAttention(e, a, batch_first=True)
            self.a_features = nn.MultiheadAttention(e, a, batch_first=True)
        else:
            self.num_heads = a
            self.head_dim = e // a
    ...
    def _forward_sdpa_qkv(self, src, sep):
        b, r, c, e = src.shape
    ...
    def forward(self, src, sep):
        if self.encoder_impl == "mha":
            return self._forward_mha(src, sep)
        return self._forward_sdpa_qkv(src, sep)
...
def init_model_from_ckpt_file(file_path):
    ...
    arch = ckpt["arch"]
    encoder_impl = arch.get("encoder_impl")
    if encoder_impl is None:
        if any("a_features" in key for key in ckpt["model"]):
            encoder_impl = "mha"
        else:
            encoder_impl = "sdpa_qkv"
...
class NanoTabPFNClassifier:
    def __init__(self, model=None, use_autocast_eval=None):
        device = "cuda"
        if model is None:
        ...
        self.device = device
        if use_autocast_eval is None:
            use_autocast_eval = c.autocast_eval
        self.use_autocast_eval = use_autocast_eval
...
model = NanoTabPFNModel(l=c.l, a=c.a, e=c.e, h=c.h, o=c.o, encoder_impl=c.encoder_impl).to(device)
...
for epoch in range(1, c.epochs + 1):
    ...
    mean_loss = (total_loss / max(num_valid, 1)).item()
    ...
    clf = NanoTabPFNClassifier(model, use_autocast_eval=c.autocast_eval)
    aucs = []
    ...
    if avg_auc >= c.jackpot:
        ckpt = {
            ...
            "arch": {
                ...
                "l": model.l,
                "o": model.o,
                "encoder_impl": model.encoder_impl,
            },
